...
fakeAsync.eachLimit = (...args) => (eachLimitOverride || async.eachLimit).apply(null, args);
...
const fakeUtil = extend({}, common_1.util);
let extended = false;
const fakePaginator = {
    paginator: {
        // tslint:disable-next-line:variable-name
...
describe('Bucket', () => {
    ...
    describe('instantiation', () => {
        ...
        it('should inherit from ServiceObject', done => {
            ...
            assert(bucket instanceof common_1.ServiceObject);
    ...
    describe('deleteFiles', () => {
        ...
        it('should delete the files', done => {
            const query = {};
            let timesCalled = 0;
            const files = [bucket.file('1'), bucket.file('2')].map(propAssign('delete', (query_, callback) => {
                timesCalled++;
        ...
        it('should execute callback with error from deleting file', done => {
            const error = new Error('Error.');
            const files = [bucket.file('1'), bucket.file('2')].map(propAssign('delete', (query, callback) => {
                callback(error);
            }));
            bucket.getFiles = (query, callback) => {
                callback(null, files);
    ...
    describe('makeAllFilesPublicPrivate_', () => {
        ...
        it('should make files public', done => {
            let timesCalled = 0;
            const files = [bucket.file('1'), bucket.file('2')].map(propAssign('makePublic', callback => {
                timesCalled++;
                callback();
            }));
            bucket.getFiles = (options, callback) => {
        ...
        it('should make files private', done => {
            ...
            let timesCalled = 0;
            const files = [bucket.file('1'), bucket.file('2')].map(propAssign('makePrivate', (options_, callback) => {
                timesCalled++;
                callback();
            }));
        ...
        it('should execute callback with files changed', done => {
            const error = new Error('Error.');
            const successFiles = [bucket.file('1'), bucket.file('2')].map(propAssign('makePublic', callback => {
                callback();
            }));
            const errorFiles = [bucket.file('3'), bucket.file('4')].map(propAssign('makePublic', callback => {
                callback(error);
            }));
            bucket.getFiles = (options, callback) => {
