...
describe('storage', () => {
        ...
        describe('private data', () => {
            ...
            it('should not upload a file', done => {
                file.save('new data', err => {
                    assert(err.message.indexOf('Could not load the default credentials') >
                        -1);
                    done();
                });
            });
                    ...
                    bucket.makePublic({ includeFiles: true }, err => {
                        ...
                        bucket.getFiles((err, files) => {
                            ...
                            async.each(files, isFilePublic, err => {
                                assert.ifError(err);
                                async.parallel([
                                    next => bucket.acl.default.delete({ entity: 'allUsers' }, next),
    ...
    describe('getting buckets', () => {
        ...
        after(done => {
            async.series(bucketsToCreate.map(bucket => {
                return done => {
    ...
    describe('requester pays', () => {
        ...
        (HAS_2ND_PROJECT ? describe : describe.skip)('existing bucket', () => {
            const storageNonWhitelist = new src_1.Storage({
            ...
            describe('methods that accept userProject', () => {
                ...
                it('bucket#combine', done => {
                    ...
                    async.each(files, createFile, err => {
                        ...
                        bucketNonWhitelist.combine(sourceFiles, destinationFile, USER_PROJECT_OPTIONS, done);
                    });
                    function createFile(fileObject, callback) {
                        fileObject.file.save(fileObject.contents, USER_PROJECT_OPTIONS, callback);
                    }
                });
                it('bucket#createNotification', doubleTest((options, done) => {
                    bucketNonWhitelist.createNotification(topicName, options, (err, _notification) => {
    ...
    describe('write, read, and remove files', () => {
        ...
        it('should copy an existing file', done => {
            ...
            bucket.upload(FILES.logo.path, opts, (err, file) => {
                ...
                file.copy('CloudLogoCopy', (err, copiedFile) => {
                    assert.ifError(err);
                    async.parallel([file.delete.bind(file), copiedFile.delete.bind(copiedFile)], done);
                });
            });
        });
        it('should copy a large file', done => {
            ...
            async.series([
                callback => bucket.upload(FILES.logo.path, { destination: file }, callback),
                callback => {
                    otherBucket.create({
                        location: 'ASIA-EAST1',
                        dra: true,
                ...
                callback => file.copy(copiedFile, callback)
            ], err => {
                assert.ifError(err);
                async.series([
